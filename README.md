...
| 图标 | 颜色 | 含义 | 要你操作吗 |
|---|---|---|---|
| `!` | 黑字**青底** | 选项询问 / 计划审批 / 权限请求。**粘性** —— 只有你输入或会话结束才清除 | **要** |
| `●` | 橙黄 | 正在跑 | 不要 |
...
的状态。但底色是**青不是红** —— 红色读作「出错了」，而 wait 的意思是
「该你了」，语义完全不同。真出错时 Claude 自己会说，不需要标签栏来喊。
...
- **告警色要按语义挑，不是按强度挑。** wait 曾经是白字红底，但红色读作
  「出错了」，而 wait 的意思是「该你了」。用青底，余光里一样抓得住，
  语义却对了。
