...
- **没有 TTL 的状态需要一个「会话还在不在」的兜底。** `busy` 和 `wait` 是仅有的
  两个不褪色的状态（`done`/`hint` 有 `--done-ttl`），设计上是对的 —— 真在等你决策
  的信号自己消失比留着更糟。但代价是 `SessionEnd` 没跑到时（Ctrl-C 硬退、终端被关、
  kill -9）它们会**永久**挂着，尤其 `wait` 带橙色底，一个早退出的会话会一直喊
  「该你了」。所以状态文件第三列记下 claude 的 pid，`kill -0` 判存活。
  记不到 pid 时**保守保留** —— 宁可留一个可能过期的状态，也不能把真在等你的 wait 误清。
  清理必须扫全部 window：会话是在自己那个 pane 里死掉的，而清理它的时机只能是
  「别处有 hook 触发」，那时当前 window 多半不是它。
- **判存活只能用 `kill -0`，不能看 messaging socket 在不在。** 早于跨会话通信的
  Claude Code 压根不建那个 socket，实测三个还在跑的 2.1.226 会话全都没有 —— 照
  socket 判会把活会话全判成死的。
- **hook 是同步阻塞 Claude 的，每一次 fork 都要算。** 加清理逻辑时顺手把
  `read_state` 里的三次 `cut` 换成一次内建 `read`，省下的 fork 比新增的清理还多，
  结果是**加了功能反而更快**（10 次 24.3ms -> 22.0ms）。状态文件没有结尾换行，
  `read` 会返回非零但变量已填好，`|| true` 吃掉即可。
- **升级只换代码不换配置，修复就只生效一半。** `install.sh` 遇到已存在的
  `glm.json` 会保留不覆盖 —— 这是对的，否则你自己改过的 `url` / `auth_scheme`
  每次升级都被冲掉。但代价是「新代码 + 老配置」：代码看得懂带过滤的写法，
  配置里却还写着裸 `percentage`，`5h` 照旧取错。而它错得很安静，数字看着完全
  正常，只是取的是另一个窗口。所以凡是「行为由配置文件驱动」的修复，都得配一个
  自动迁移（`migrateGLMFields`），靠 release notes 让人手动改，半年后等于没改。
  迁移只替换**精确等于**老名字的那一项、其余候选原样保留，且一项都没命中就不落盘 ——
  否则每次渲染都写一遍文件，又攒出一堆内容相同的 `.bak`。
